...
namespace jbriggs::common::display
{
  /**
  ...
  class LCDDisplay : public IDisplay
  {
    ...
    void displayFlashMessage(const std::string& rowOne, const std::string& rowTwo) const;
    void displayScrollMessage(const std::string& rowOne, const std::string& rowTwo) const;
...
public:
...
#else
    static constexpr int FLASH_DISPLAY_TIME_MS = 1000;
     ...
     */
    void displayText(std::string rowOne, std::string rowTwo, PrintType printType) override;
     ...
     */
    void displayText(std::string rowOne, PrintType printType) override;
